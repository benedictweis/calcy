use crate::decimal::Decimal;
use std::str::FromStr;

#[test]
fn parse() {
    assert_eq!(Decimal::from_str("0"), Ok(Decimal::new(0, 0, false)));
    assert_eq!(Decimal::from_str("1"), Ok(Decimal::new(1, 0, false)));
    assert_eq!(Decimal::from_str("1.1"), Ok(Decimal::new(11, 1, false)));
    assert_eq!(Decimal::from_str("11.11"), Ok(Decimal::new(1111, 2, false)));
    assert_eq!(Decimal::from_str("11.111"), Ok(Decimal::new(11111, 3, false)));
    assert_eq!(Decimal::from_str("1.2345"), Ok(Decimal::new(12345, 4, false)));
}

#[test]
fn display() {
    assert_eq!(Decimal::from_str("0").unwrap().to_string(), "0".to_string());
    assert_eq!(Decimal::from_str("1").unwrap().to_string(), "1".to_string());
    assert_eq!(Decimal::from_str("1.1").unwrap().to_string(), "1.1".to_string());
    assert_eq!(Decimal::from_str("11.11").unwrap().to_string(), "11.11".to_string());
    assert_eq!(Decimal::from_str("11.111").unwrap().to_string(), "11.111".to_string());
    assert_eq!(Decimal::from_str("1.2345").unwrap().to_string(), "1.2345".to_string());
}
